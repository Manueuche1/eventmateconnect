...
export function BottomTabs() {
  ...
  return (
    <nav className="shrink-0 border-t border-brand-mist bg-white px-2 pt-2 pb-3 grid grid-cols-4">
      {tabs.map(t => {
        ...
        return (
          <Link key={t.to} to={t.to} className="flex flex-col items-center gap-1 py-1">
            ...
            <span className={`text-[11px] font-medium ${active ? "text-brand-indigo" : "text-brand-slate"}`}>{t.label}</span>
