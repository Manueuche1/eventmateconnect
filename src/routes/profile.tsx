...
function Profile() {
  useRequireAuth();
  const { resetAll, setRole } = useEventMate();
  const { profile, user, signOut } = useAuth();
  const navigate = useNavigate();
  const initials = profile?.avatar_initials || (profile?.full_name ?? "U").slice(0, 2).toUpperCase();
    ...
    <PhoneFrame>
      <div className="flex-1 overflow-y-auto">
        <div className="bg-brand-indigo text-white px-6 pt-8 pb-6 flex items-center gap-4">
          <div className="h-16 w-16 rounded-full bg-brand-amber text-brand-indigo grid place-items-center text-xl font-semibold">{initials}</div>
          <div>
            <div className="text-lg font-semibold">{profile?.full_name ?? "—"}</div>
            <div className="text-xs text-white/70 mt-0.5 capitalize">{profile?.role ?? ""}</div>
        ...
        <div className="p-4 space-y-2">
          ...
          {[
            ...
            { icon: Lock, label: "Privacy and data", onClick: () => {} },
            { icon: HelpCircle, label: "Help", onClick: () => {} },
            { icon: LogOut, label: "Sign out", onClick: handleSignOut },
          ].map(({ icon: Icon, label, onClick }) => (
            <button key={label} onClick={onClick} className="w-full px-4 py-3 rounded-xl bg-white border border-brand-mist flex items-center gap-3 text-sm">
